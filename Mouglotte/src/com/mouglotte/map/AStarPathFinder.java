...
public class AStarPathFinder implements PathFinder {
	...
	public AStarPathFinder(TileBasedMap map, int maxSearchDistance,
		...
		this(map, maxSearchDistance, allowDiagMovement,
				(AStarHeuristic) new ClosestHeuristic());
	}
			...
			for (int x = -1; x < 2; x++) {
				for (int y = -1; y < 2; y++) {
					...
					if (isValidLocation(mover, sx, sy, xp, yp)) {
						...
						// current plus the movement cost to reach this node.
						// Note that the heuristic value is only used in the
						// sorted open list
						...
						// lower than it has been previously, makes sure the
						// node hasn'e've determined that there might have been
						...
						// discarded then reset it's cost to our current cost
						// and add it as a next possible step (i.e. to the open
						// list)
						if (!inOpenList(neighbour)
